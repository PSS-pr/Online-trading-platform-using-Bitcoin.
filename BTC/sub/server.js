...
app.get('/exchange', (req, res) => {
  ...
  axios(accountOptions)
    .then(accountResponse => {
      const jsonData = accountResponse.data;
      const formattedValues = jsonData.map(obj => Object.values(obj));
      console.log("FORS",formattedValues);
      const marketOptions = {
        method: 'GET',
...
app.post('/sell-order', (req, res) => {
  const market = 'KRW-' + req.body.selectedMarket;
  console.log(market);
  const side = 'ask'; 
...
io.on('connection', async (socket,req,res) => {
  ...
  mysqlConnection.query(insertMessageQuery, [tableID, sellerId, senderID,price])
  .then(() => {
...
app.get('/market',async (req, res) => {app.set('view engine', 'ejs');
    ...
    try {
      ...
      const query = `
        SELECT goods.goods_id, goods.title, images.url ,state
        FROM goods
...
app.get('/goods/search', async (req, res) => {
  const user_id = req.session.user_id; // 세션에서 user_id 가져오기
  const bitcoinPrice = await getBitcoinPrice2();
  console.log("AOB",bitcoinPrice);
  if (!user_id) {
    return res.redirect('/user/login');
  }
  ...
  try {
    ...
    res.render('goods', { searched_goods, keywords,bitcoinPrice });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, message: '서버 오류' });
  }
...
app.get('/getBitcoinPrice2', async (req, res) => {
  const bitcoinPrice = await getBitcoinPrice2();
  res.json({ bit: bitcoinPrice });
});

wss.on('connection', (ws) => {
  // WebSocket 연결이 생성되면 2초마다 비트코인 가격을 전송
  setInterval(async () => {
    const bitcoinPrice = await getBitcoinPrice2();
    ws.send(JSON.stringify({ bit: bitcoinPrice }));
  }, 2000);
});
